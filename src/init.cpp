...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: XUV.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: XUVd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeXUVamount=<n> " + _("Keep N XUV anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. XUV is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. XUV is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("XUV version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "XUV server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of XUV") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart XUV to complete") << "\n";
    ...
    nAnonymizeXUVAmount = GetArg("-anonymizeXUVamount", 0);
    if(nAnonymizeXUVAmount > 999999) nAnonymizeXUVAmount = 999999;
    if(nAnonymizeXUVAmount < 2) nAnonymizeXUVAmount = 2;
    ...
    LogPrintf("Anonymize XUV Amount %d\n", nAnonymizeXUVAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1XUV+1000 == (.1XUV+100)*10
       10XUV+10000 == (1XUV+1000)*10
    */
