...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XUV
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XUV
    // Mac: ~/Library/Application Support/XUV
    // Unix: ~/.XUV
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XUV";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "XUV.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "XUVd.pid"));
