...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeXUVAmount"))
        settings.setValue("nAnonymizeXUVAmount", 1000);
    nAnonymizeXUVAmount = settings.value("nAnonymizeXUVAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeXUVAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeXUVAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeXUVAmount:
            return QVariant(nAnonymizeXUVAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeXUVAmount:
            nAnonymizeXUVAmount = value.toInt();
            settings.setValue("nAnonymizeXUVAmount", nAnonymizeXUVAmount);
            emit AnonymizeXUVAmountChanged(nAnonymizeXUVAmount);
            break;
        default:
            break;
        }
